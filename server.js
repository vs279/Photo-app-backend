...
// Configure Cloudinary for permanent photo storage (optional but recommended).
...
if (cloudinaryReady) {
  ...
  console.log("Cloudinary storage: not configured (photos kept in memory)");
}
...
// Cancel a host's subscription immediately (used when they delete their account).
app.post("/subscription/cancel", async (req, res) => {
  try {
    const sub = db.subscriptions[req.body.hostId];
    if (sub?.subscriptionId) {
      try { await stripe.subscriptions.cancel(sub.subscriptionId); } catch (e) { /* may already be gone */ }
    }
    delete db.subscriptions[req.body.hostId];
    res.json({ ok: true });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
// ============================================================================
// Photos are retained for TWO MONTHS from the last time the album was updated.
const RETENTION_MS = 60 * 24 * 60 * 60 * 1000; // ~2 months (60 days)
...
app.post("/album/save", async (req, res) => {
  ...
  const now = Date.now();
  const existing = db.albums[album.id];

  let photos = Array.isArray(album.photos) ? album.photos : [];

  // If Cloudinary is set up, upload any base64 images and keep just the URLs.
  // This makes photos permanent (survive restarts) and keeps memory tiny.
  if (cloudinaryReady) {
    photos = await Promise.all(photos.map(async (p) => {
      const out = { ...p };
      try {
        // Only upload if it's still a raw base64 data URL (not already a link).
        if (typeof p.src === "string" && p.src.startsWith("data:")) {
          const up = await cloudinary.uploader.upload(p.src, { folder: `photoapp/${album.id}` });
          out.src = up.secure_url;
        }
        if (typeof p.cleanSrc === "string" && p.cleanSrc.startsWith("data:")) {
          const up2 = await cloudinary.uploader.upload(p.cleanSrc, { folder: `photoapp/${album.id}/clean` });
          out.cleanSrc = up2.secure_url;
        }
      } catch (e) {
        console.error("Cloudinary upload failed for a photo:", e.message);
        // keep original base64 as a fallback so nothing is lost
      }
      return out;
    }));
  }
  ...
  db.albums[album.id] = {
    ...
    updatedAt: now,
    expiresAt: now + RETENTION_MS, // photos kept for 2 months from last update
  };
  res.json({ ok: true, expiresAt: db.albums[album.id].expiresAt, album: db.albums[album.id] });
});

// ── Host settings (shared across all devices) ────────────────────────────────
// hostId -> { watermarkEnabled }
db.hostSettings = db.hostSettings || {};

app.post("/host/settings", (req, res) => {
  const { hostId, watermarkEnabled } = req.body;
  if (!hostId) return res.status(400).json({ error: "hostId required" });
  db.hostSettings[hostId] = { watermarkEnabled: watermarkEnabled !== false };
  // Keep existing albums in step so guests see the change immediately.
  Object.values(db.albums).forEach(a => {
    if (a.hostId === hostId) a.watermarkEnabled = watermarkEnabled !== false;
  });
  res.json({ ok: true });
});

app.get("/host/settings", (req, res) => {
  const s = db.hostSettings[req.query.hostId];
  res.json({ watermarkEnabled: s ? s.watermarkEnabled : true });
});

// List all albums belonging to a host (so they reappear after logging back in).
app.get("/albums", (req, res) => {
  const hostId = req.query.hostId;
  if (!hostId) return res.status(400).json({ error: "hostId required" });
  const now = Date.now();
  const albums = Object.values(db.albums).filter(
    (a) => a.hostId === hostId && (!a.expiresAt || now <= a.expiresAt)
  );
  res.json({ albums });
});

// Guest fetches an album by its code (works on any device, anywhere).
// Photos remain available for 2 months even if the host account is deleted.
app.get("/album/:code", (req, res) => {
  const code = (req.params.code || "").toUpperCase();
  const album = db.albums[code];
  if (!album) return res.status(404).json({ error: "not found" });
  if (album.expiresAt && Date.now() > album.expiresAt) {
    delete db.albums[code]; // expired — remove it
    return res.status(410).json({ error: "expired" });
  }
  res.json({ album });
});

// Periodic cleanup: delete albums whose 2-month window has passed.
setInterval(() => {
  const now = Date.now();
  for (const [code, a] of Object.entries(db.albums)) {
    if (a.expiresAt && now > a.expiresAt) delete db.albums[code];
  }
}, 60 * 60 * 1000); // hourly
